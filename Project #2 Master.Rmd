...
These factors were significant in our logistic model, but not our regression model:
...
#Linear Regression Arbitrage Conclusion
...
Here, we are looking for items that are significant in the regression model, but not in the other, or for significant items that are much different in importance in one model vs. another. 

1. Home ownership 
Home ownership is significant in our logistic model, but not in our linear model. It might be fair to assume that people would not consider someone who owns a home to be a risky borrower (which is evidenced by our linear model)
however, our logistic model shows us that these borrowers are more likely to default on their loans than a non-homeowner. In an investment setting, this would mean we are likely to not invest in home owners as their added risk is not priced into the interest rate. 